...
1. Clash Party → 覆写 → 新建 → 类型选**远程**、格式选 **JavaScript**
2. 地址填：
...
> ⚠️ 如果同时使用其他全局覆写（如订阅转换脚本），组会叠加冲突，建议二选一。

## 生成的组结构

| 组 | 类型 | 说明 |
|---|---|---|
| 自动选择 | url-test | 候选为各地区组，组内再自动选最快节点（120s 测速 / 3s 超时 / 200ms 容差 / lazy / 失败即重选） |
...
| 国外媒体 | select | 候选：自动 → 手动 → 香港 → 美国 → 台湾 → 日本 → 新加坡；分流：Netflix、YouTube、Disney、PrimeVideo、HBO、TikTok、Spotify |
| 国内媒体 | select | 默认 DIRECT（B站、爱奇艺、优酷直连），可切自动/手动 |
| Apple | select | 默认 DIRECT，可切自动/手动或各地区；分流：apple、icloud |
| Google | select | 候选：自动 → 手动 → 香港 → 美国；分流：google（含 googleapis、gstatic 等，不含 YouTube） |
| Microsoft | select | 默认 DIRECT，可切自动/手动或香港/美国；分流：microsoft、bing（微软国内服务 microsoft@cn 强制直连） |
| GitHub | select | 候选：自动 → 手动 → 香港 → 美国；分流：github |
| Final | select | 兜底：自动 / 手动 / 各地区 / DIRECT，规则最后 MATCH 进它 |
| 地区分组 | url-test | 香港、台湾、日本、新加坡、韩国、美国，按节点名正则自动归类，空地区自动隐藏 |

规则顺序：局域网、jsdelivr 与 Cloudflare R2 直连 → 苹果、微软国内服务、国内 AI 直连 → 国内媒体 → Apple → AI服务 → 加密货币 → 国外媒体 → Google → Microsoft → GitHub → 中国大陆直连 → Final 兜底
...
本仓库仅供个人学习、研究与配置整理使用，不提供任何节点、订阅链接或网络服务，脚本本身不连接任何服务器。脚本按“现状”提供，作者不对其正确性、可用性或稳定性作任何明示或默示担保；使用本脚本时添加的订阅、节点及由此产生的全部网络行为，由使用者本人负责，请遵守所在国家/地区的法律法规；一切直接或间接后果（包括但不限于服务中断、账号封禁、法律风险与财产损失）均由使用者自行承担，与仓库作者无关。下载、复制或使用本仓库任何内容，即视为已阅读并同意接受本声明全部条款；不同意者请立即停止使用并删除相关内容。
